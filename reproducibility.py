...
from tools import announce_msg, check_if_allow_multgpu_mode
...
def get_seed():
    ...
    try:
        ...
        print(
            "In Bash, you need to create an environment variable of the seed "
            "named `MYSEED`, then set its value to an integer.\n"
            "For example, to create an environment named `MYSEED` and set it "
            "to the value 0, in your Bash terminal, "
            "before running this script, type: `export MYSEED=0`.")
        print(" .... [NOT OK]")

        warnings.warn("WE ARE GOING TO USE OUR DEFAULT SEED:"
                      " {}  .... [NOT OK]".format(DEFAULT_SEED))
